...
from dhanet.data import DHANetDataset, collate, read_manifest
from dhanet.model import DHANet
...
def run_epoch(model, loader, optimizer, device, args, collect_outputs=False, debug_shapes=False):
    ...
    for batch in iterator:
        ...
        with torch.set_grad_enabled(train):
            ...
            debug_once = False
            cls1 = focal_loss(out["backbone_logits"], target, args.focal_gamma)
            cls2 = focal_loss(out["graph_logits"], target, args.focal_gamma)
            loss = cls1 + cls2 + args.gamma_node * out["node_loss"] + args.gamma_edge * out["edge_loss"]
            if train:
...
def train_fold(args, frame, train_idx, val_idx, fold, label_to_idx):
    train_ds = DHANetDataset(frame.iloc[train_idx], args.input_shape, augment=True)
    val_ds = DHANetDataset(frame.iloc[val_idx], args.input_shape, augment=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        row = {"epoch": epoch, "lr": optimizer.param_groups[0]["lr"], "train": train_stats, "val": val_stats}
...
def main():
    parser = argparse.ArgumentParser(description="Reproduce DHANet on local ADNI sMRI data.")
    ...
    parser.add_argument("--output-dir", default="dhanet_runs")
    parser.add_argument("--input-shape", type=parse_shape, default=(64, 80, 64))
    parser.add_argument("--graph-shape", type=parse_shape, default=(4, 5, 4))
    ...
    parser.add_argument("--gamma-node", type=float, default=1.0)
    parser.add_argument("--gamma-edge", type=float, default=0.8)
